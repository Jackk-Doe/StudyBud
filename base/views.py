...
def createRoom(request):
    ...
    if request.method == 'POST':
        ...
        # print(form)
        if form.is_valid():
            room = form.save(commit=False)  #Return the Room, Not yet save
            room.host = request.user    #Room's host is who ever logging in
            room.save()
            return redirect('home')
...
def updateRoom(request, pk):
    ...
    if request.method == 'POST':
        # Update from [request.POST] <- [instance=room]
